...
export const DashboardChart: React.FC<DashboardChartProps> = ({
  ...
  return (
    <motion.div 
      ...
      className="bg-white dark:bg-slate-900/90 rounded-3xl p-6 sm:p-7 border border-slate-200/80 dark:border-slate-800 shadow-sm relative overflow-hidden"
    >
      {/* Top Header Section */}
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6 pb-5 border-b border-slate-100 dark:border-slate-800">
        <div className="flex items-center gap-3">
          <div className="p-3 bg-emerald-50 dark:bg-emerald-950/60 text-emerald-600 dark:text-emerald-400 rounded-2xl border border-emerald-100 dark:border-emerald-900/50 shadow-sm shrink-0">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="inline-flex items-center gap-1 text-[11px] font-bold text-emerald-700 dark:text-emerald-300 bg-emerald-100/80 dark:bg-emerald-950/80 px-2.5 py-0.5 rounded-full border border-emerald-200 dark:border-emerald-800/60">
        ...
        <div className="flex items-center gap-4 flex-wrap justify-between lg:justify-end">
          <div className="flex items-center gap-3 text-xs font-bold bg-slate-50 dark:bg-slate-800/50 px-3.5 py-2 rounded-2xl border border-slate-200/60 dark:border-slate-700/60">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
        <div className="p-3.5 bg-slate-50/80 dark:bg-slate-800/40 rounded-2xl border border-slate-100 dark:border-slate-800 flex items-center justify-between">
          <div className="space-y-0.5">
            <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider font-mono">Média Mensal</span>
            <p className="text-sm font-black text-slate-900 dark:text-white font-mono">{formatCurrency(avgMonthlyReceived)}</p>
          </div>
          <div className="w-8 h-8 rounded-xl bg-emerald-100/60 dark:bg-emerald-950/60 text-emerald-600 dark:text-emerald-400 flex items-center justify-center">
        ...
        <div className="p-3.5 bg-slate-50/80 dark:bg-slate-800/40 rounded-2xl border border-slate-100 dark:border-slate-800 flex items-center justify-between">
          <div className="space-y-0.5">
            ...
            <p className="text-sm font-black text-slate-900 dark:text-white font-mono">{formatCurrency(grandTotal)}</p>
          </div>
          <div className="w-8 h-8 rounded-xl bg-teal-100/60 dark:bg-teal-950/60 text-teal-600 dark:text-teal-400 flex items-center justify-center">
        ...
        <div className="p-3.5 bg-slate-50/80 dark:bg-slate-800/40 rounded-2xl border border-slate-100 dark:border-slate-800 flex items-center justify-between">
          <div className="space-y-0.5">
            ...
            <p className="text-sm font-black text-emerald-600 dark:text-emerald-400 font-mono">{settlementRate}% quitado</p>
          </div>
          <div className="w-8 h-8 rounded-xl bg-indigo-100/60 dark:bg-indigo-950/60 text-indigo-600 dark:text-indigo-400 flex items-center justify-center">
      ...
      {chartMode === 'bar' ? (
        <div className="pt-2">
          <div className="h-64 sm:h-72 w-full flex items-end gap-2 sm:gap-6 pt-12 pb-2 px-2 relative border-b border-slate-100 dark:border-slate-800">
            {/* Horizontal guideline grid */}
            <div className="absolute inset-x-0 top-0 border-b border-dashed border-slate-200/60 dark:border-slate-800" />
            <div className="absolute inset-x-0 top-1/3 border-b border-dashed border-slate-200/60 dark:border-slate-800" />
            <div className="absolute inset-x-0 top-2/3 border-b border-dashed border-slate-200/60 dark:border-slate-800" />
                ...
                >
                  ...
                  <div className="w-full flex items-end justify-center gap-1.5 sm:gap-2 h-full max-w-[90px]">
                    ...
                    <div className="flex-1 flex flex-col items-center h-full justify-end">
                      {item.received > 0 && (
                        <span className="text-[10px] font-mono text-emerald-600 dark:text-emerald-400 font-bold mb-1 opacity-0 group-hover:opacity-100 transition-opacity hidden sm:block">
                    ...
                    <div className="flex-1 flex flex-col items-center h-full justify-end">
                      {item.pending > 0 && (
                        <span className="text-[10px] font-mono text-amber-600 dark:text-amber-400 font-bold mb-1 opacity-0 group-hover:opacity-100 transition-opacity hidden sm:block">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 pt-2">
          {monthlyChartData.map((item, idx) => {
            ...
            return (
              <div key={`${item.monthKey}-${idx}`} className="p-4 bg-slate-50/80 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800 flex flex-col justify-between hover:border-emerald-300 dark:hover:border-emerald-800 transition-all duration-200">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-black uppercase text-slate-800 dark:text-slate-200 tracking-wider font-mono">
                    {item.label} ({item.monthKey})
                  </span>
                ...
                <div className="space-y-1.5 my-2">
                  <div className="flex justify-between text-xs font-medium">
                    ...
                    <span className="font-bold text-emerald-600 dark:text-emerald-400 font-mono">{formatCurrency(item.received)}</span>
                  </div>
                  <div className="flex justify-between text-xs font-medium">
                    <span className="text-slate-500 dark:text-slate-400">Pendente:</span>
                    <span className="font-bold text-amber-600 dark:text-amber-400 font-mono">{formatCurrency(item.pending)}</span>
                  </div>
                </div>
      ...
      {/* Footer Info */}
      <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs text-slate-400 dark:text-slate-500">
