...
export const DashboardUpcomingDebts: React.FC<DashboardUpcomingDebtsProps> = ({
  ...
  return (
    ...
    >
      <div className="w-full bg-white/90 rounded-3xl p-6 sm:p-7 border border-slate-200/80 shadow-sm flex flex-col justify-between">
        <div>
          {/* Header Bar */}
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 pb-4 border-b border-slate-100">
            <div>
              ...
              <h3 className="text-xl font-black text-slate-900 tracking-tight mt-0.5 font-display">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <button
                ...
                className="text-xs font-extrabold text-emerald-700 hover:text-emerald-800 bg-emerald-50/60 hover:bg-emerald-100/60 px-3.5 py-2 rounded-2xl border border-emerald-200/60 transition-all duration-200 cursor-pointer flex items-center gap-1 shrink-0"
              >
                <span>Ver Todos</span>
          ...
          {upcomingDebts.length > 3 && (
            <div className="relative mb-4">
              ...
              <input
                ...
                className="w-full bg-slate-50/50 border border-slate-200/80/80 pl-10 pr-4 py-2.5 rounded-2xl text-xs text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all"
              />
            </div>
          )}
          ...
          {filteredDebts.length > 0 ? (
                ...
                {filteredDebts.map((debt, index) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className="group p-4 sm:p-4.5 bg-slate-50/70/40 hover:bg-white rounded-2xl border border-slate-200/80 hover:border-emerald-300 shadow-2xs hover:shadow-md transition-all duration-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4"
                    >
                      <div className="min-w-0 flex-1 flex items-center gap-3.5">
                        ...
                        <div className="min-w-0 space-y-1">
                          <div className="flex items-center gap-2 flex-wrap">
                            <h4 className="text-sm font-black text-slate-900 truncate font-display">{debt.name}</h4>
                          ...
                          <div className="flex items-center gap-3 text-xs text-slate-500 flex-wrap">
                            <span className="flex items-center gap-1 font-medium">
                              ...
                              Vence: <strong className="text-slate-800 font-mono">{formatDate(debt.dueDate!)}</strong>
                            </span>
                            {debt.description && (
                      ...
                      <div className="flex items-center justify-between sm:justify-end gap-3 shrink-0 pt-2 sm:pt-0 border-t sm:border-t-0 border-slate-200/60">
                        <span className="text-base font-black text-slate-900 mr-1 font-mono">
                        ...
                        {debt.phone && (
                          <a
                            ...
                            className="p-2 bg-emerald-50/60 hover:bg-emerald-500 text-emerald-700 hover:text-white border border-emerald-200/80 rounded-xl transition-all duration-200 cursor-pointer flex items-center justify-center shadow-xs hover:shadow-emerald-500/20 active:scale-95"
            ...
            <div className="py-12 text-center text-slate-400 bg-slate-50/80/40 rounded-2xl border border-dashed border-slate-200">
              ...
              <p className="text-[11px] text-slate-400 mt-0.5">Todas as suas cobranças estão quitadas ou em dia no filtro selecionado.</p>
            </div>
          )}
        </div>

        {/* Footer Legend */}
        <div className="mt-5 pt-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500 flex-wrap gap-2">
